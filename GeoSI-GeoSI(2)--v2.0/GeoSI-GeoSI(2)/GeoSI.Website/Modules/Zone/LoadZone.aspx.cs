...
namespace GeoSI.Website.Modules.Zone
{
    public partial class LoadZone : Form
    {
        ...
        public void Vider(object sender, DirectEventArgs e)
        {
            ...
            int cpt = MultiCombo1.SelectedItems.Count();

            List<string> tab = new List<string>();
            List<string> tabv = new List<string>();



            for (int i = 0; i < cpt; i++)
            {
                tab.Add(MultiCombo1.SelectedItems.ElementAt(i).Text);
                tabv.Add(MultiCombo1.SelectedItems.ElementAt(i).Value);
            }
            string d="";
            string v = "";
...
//   X.Msg.Alert("Selected value >>>>> : ", v).Show();
            ...
            if (_zoneid == "")
            {
                ...
                if (groupes.Length > 0)
                {
                    for (int i = 0; i < groupes.Length; i++)
                    {
                     ...
                     SqlDataReader dr=   p.Select(req_lat);
                     while (dr.Read())
                     {
                         _grpid =(int) dr[0];
                     }
                ...
                for (int i = 0; i < cpt; i++)
                {
                    tab.Add(MultiCombo1.SelectedItems.ElementAt(i).Value);

                }
                for (int i = 0; i < tab.Count; i++)
                {
                    p.Insert("insert into Vehicule_Zone (vehiculeid,zoneid,interdit,clientid,actif)  values('" + tab[i].ToString() + "','" + _zoneid + "','" + _interdit + "','"+this.getCurrentUser().getClientId()+"',1)");
                }
                string _req11 = "SELECT zoneid,nom_zone,tolerance,interdit, replace(replace(replace(replace(replace(polygone.STAsText(),'LINESTRING (','newgoogle.maps.LatLng('),',',');newgoogle.maps.LatLng('),'( ','('),' ',','),'newgoogle','new google') as  polygone FROM zone where clientid=" + this.getCurrentUser().getClientId() + " and actif='1' order by zoneid desc";
                this.FillStore(_req11, this.StoreZone);
