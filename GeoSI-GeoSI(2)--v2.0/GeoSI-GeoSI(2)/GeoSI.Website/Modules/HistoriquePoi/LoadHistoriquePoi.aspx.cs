    ...
    public partial class LoadHistoriquePoi : Page
    {
        ...
        protected void HistoriquePoi(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    string _reqTime = "select * from(select top 1 d.RTCDateTime from Historique_Datatracker  d order by d.datatrackerid desc) a union select * from (select top 1 d.RTCDateTime from Historique_Datatracker d order by d.datatrackerid )b ";
                    ...
                    else
                    {
                     ...
                     //+ " and p.actif='1' and p.clientid='" + this.getCurrentUser().getClientId() + "' and p.userid ='" + this.getCurrentUser().getUserId() + "'  and d.speed=0) aff where aff.inter=1   ";
                        ...
                        string re = "select p.libelle,p.type_poi,p.longitude,p.latitude,h.date_debut,h.date_fin,"
       +" per.prenom+' '+per.nom as conducteur,h.duree from Historique_Poi h"
     +"  inner join Poi p on p.poiid=h.poid"
     +"  inner join personnel per on per.personnelid =h.personelid and per.actif=1"
     + "  where h.date_debut  >'" + userDate1 + "'  and h.date_debut<='" + userDate2 + "'  "
     + "  and h.vehiculeid='" + vehiculeid + "'  and p.clientid='" + this.getCurrentUser().getClientId() + "' ";


                        SqlDataReader drlist = Select(re);
                        ...
                        while (drlist.Read())
                        {
                            ...
                            t.dateF = drlist[5].ToString();
                            t.cond = drlist[6].ToString();
                            t.duree = drlist[7].ToString();
        ...
        protected void HistoriquePoi1(object sender, EventArgs e)
        {
