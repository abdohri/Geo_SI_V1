...
{
    public partial class VehEnAlarmeLoad : Page
    {
        ...
        public string NodeLoad(string nodeID)
        {
            ...
            if (!string.IsNullOrEmpty(nodeID))
            {
                ...
                string _reqVeh = " select * from  (select *, "
+"  (select top 1 contact from Datatracker where imei = aff2.imei order by datatrackerid desc) as dernier_contact ,"
+" (select top 1 signialGPS  from Datatracker where imei = aff2.imei order by datatrackerid desc) as signal "
 +" from (select aff.* from "
+"   (select v.vehiculeid, v.matricule, v.typevehiculeid , isnull(gv.groupeid, 1) as groupeid ,"
+"   b.imei from vehicules v "
 ...
 +"   inner join Notification nt on nt.vehiculeid=abv.vehiculeid"
 +"    inner join boitier b on b.boitierid=abv.boitierid "
 + "    where v.clientid = " + this.getCurrentUser().getClientId() + "  and v.actif = '1' and nt.Vu='non' )aff "
 +"     union  "
 +"     select v.vehiculeid, v.matricule, v.typevehiculeid ,1 as groupeid  , b.imei from vehicules v"
  +"      inner join  profil_vehicule pv on pv.vehiculeid = v.vehiculeid and pv.actif = '1' "
  +"       inner join profil p on p.profilid = pv.profilid and p.actif ='1'  "
  +"        inner join affectation_vehicule_boitier abv on abv.vehiculeid=v.vehiculeid "
  +"         inner join boitier b on b.boitierid=abv.boitierid  "
   +"        inner join Notification nt on nt.vehiculeid=abv.vehiculeid "
   +"         inner join profil_user pu on pu.actif = '1' and pu.profilid = p.profilid "
   + "         where  pu.utilisateurid = " + this.getCurrentUser().getUserId() + " and  v.clientid =  " + this.getCurrentUser().getClientId() + " and v.actif = '1'  and nt.Vu='non') aff2 "
      + "       where groupeid =" + nodeID + ")t where t.dernier_contact is not null";
...
/*
 ...
 + " from vehicules v"
  + " left outer join groupe_vehicule gv on gv.vehiculeid = v.vehiculeid and gv.actif = '1'"
  + " inner join affectation_vehicule_boitier abv on abv.vehiculeid=v.vehiculeid   inner join Notification nt on nt.vehiculeid=abv.vehiculeid "
   + " inner join boitier b on b.boitierid=abv.boitierid  inner join Datatracker da on da.imei=b.imei "
   + " where v.clientid = " + this.getCurrentUser().getClientId() + " and v.actif = '1' and nt.Vu='non'  )aff "
   + " union "
    + " select v.vehiculeid, v.matricule, v.typevehiculeid ,1 as groupeid  , b.imei from vehicules v  inner join  profil_vehicule pv on pv.vehiculeid = v.vehiculeid and pv.actif = '1'  inner join profil p on p.profilid = pv.profilid and p.actif ='1'   inner join affectation_vehicule_boitier abv on abv.vehiculeid=v.vehiculeid  inner join boitier b on b.boitierid=abv.boitierid  inner join Notification nt on nt.vehiculeid=abv.vehiculeid   inner join Datatracker da on da.imei=b.imei inner join profil_user pu on pu.actif = '1' and pu.profilid = p.profilid where  pu.utilisateurid = " + this.getCurrentUser().getUserId() + " and  v.clientid = " + this.getCurrentUser().getClientId() + " and v.actif = '1'  and nt.Vu='non'  ) aff2  where groupeid =" + nodeID + ")t where t.dernier_contact is not null    ";
                */
